...
import {
  persistenceUpdateCurrent,
  persistenceUpdateSelected,
...
export const setCountry = (country) => (dispatch, getState) => {
  ...
  if (
    Object.keys(getState().country.listOfCountries).length === 0 &&
    !getState().country.loadingCountries
  ) {
  ...
  if (
    ...
    if (!getState().country.loadingCountries) {
      ...
      try {
        ...
        const sortedCountriesObject = sortedCountries.reduce(
          (obj, item) => ((obj[item.iso] = item.name), obj),
          {}
        );
        dispatch({
...
export const getCurrentData = (country) => async (dispatch, getState) => {
  if (
    !getState().country.currentData[country] &&
    country &&
    !getState().country.loadingCurrent[country]
  ) {
    dispatch({
    ...
    try {
      ...
      aggregatedData.vaccinated_per_hundred += parseFloat(
        vacdata.people_fully_vaccinated_per_hundred
      );
